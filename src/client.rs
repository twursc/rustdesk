...
// OHOS v0.1：scrap::record cfg-out，提供本地 stub 以兜底类型签名
#[cfg(target_env = "ohos")]
pub struct Recorder;
#[cfg(target_env = "ohos")]
...
#[cfg(target_env = "ohos")]
impl Recorder {
    pub fn new(_ctx: RecorderContext) -> Result<Self, ()> {
        Err(())
    }
    pub fn write_frame<T>(&mut self, _frame: T, _w: usize, _h: usize) -> Result<(), ()> {
        Err(())
    }
}
